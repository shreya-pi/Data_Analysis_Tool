...
class FunctionComparer:
    def __init__(self):
        ...
        with open(self.json_path, 'r') as f:
            self.function_tests = json.load(f)

    def fetch_function_result(self, query: str, platform: str) -> pd.DataFrame:
...
# Not needed for functions as they are provided in the JSON input
    # @staticmethod
    ...
    def compare_all_functions(self):
        comparer = Compare()
        print(f"🔍 Found {len(self.function_tests)} functions to compare.\n")

        for func_name, queries in self.function_tests.items():
            try:
                sql_query = queries.get("sql_function_query")
                sf_query = queries.get("sf_function_query")

                if not sql_query or not sf_query:
                    print(f"⚠️ Skipping `{func_name}` due to missing queries.")
                    continue

                df_sql = self.fetch_function_result(sql_query, 'sqlserver')
                df_sf = self.fetch_function_result(sf_query, 'snowflake')

                comparer.compare_results(
                    df_sf, df_sql, snowflake_name=func_name, sqlserver_name=func_name, entity_type= 'function')
                
                # self.compare_results(df_sf, df_sql, func_name)
            except Exception as e:
                print(f"⚠️ Error comparing function `{func_name}`: {e}")

        print("\n🔍 Comparison completed.")

if __name__ == "__main__":
    comparer = FunctionComparer()
    comparer.compare_all_functions()
